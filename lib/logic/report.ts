...
export function generateCultivationReport(records: LocalRecord[], startYear: number, startMonth: number, months: number): string {
  ...
  if (envRecs.length > 0) {
    ...
    for (const r of envRecs) {
      ...
      if (hd.max_temp != null || hd.min_temp != null) {
        const maxStr = hd.max_temp != null ? `最高${hd.max_temp}℃` : '-';
        const minStr = hd.min_temp != null ? `最低${hd.min_temp}℃` : '-';
        const loc = r.location || '未指定';
        rpt += `  ${r.date} ${loc}: ${maxStr} / ${minStr}\n`;
      }
    }
...
export function generatePesticideReport(records: LocalRecord[], startYear: number, startMonth: number, months: number): string {
  const start = new Date(startYear, startMonth, 1);
  const end = new Date(startYear, startMonth + months, 0);

  const rangeRecs = records.filter(r => {
    const d = new Date(r.date);
    return d >= start && d <= end && r.pesticide_detail && r.pesticide_detail.trim();
  }).sort((a, b) => String(a.date).localeCompare(String(b.date)));
  ...
  for (const r of rangeRecs) {
    const loc = r.location || '未指定';
    rpt += `  ${r.date}  ${loc.padEnd(10, '　')}${r.pesticide_detail}\n`;
  }
  rpt += `─────────────────────────────\n`;
  rpt += `  合計散布回数: ${rangeRecs.length}回\n`;
