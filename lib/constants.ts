...
- 元の入力フィールド値（work_log, fertilizer等）は変更不可。ただし生成テキスト（admin_log, narrative, insight, reply, advice）では全て補正済みの語を使うこと
...
- あくまで概算。正確な市場価格ではない旨をadviceで触れる必要はない（数字だけ返す）`,
  ...
  NARRATIVE_RULES: `【narrative / insight 生成ルール】
■ narrative（今日の振り返り日記、150文字以内）:
- 「相棒」として今日の作業への共感と労いを書く温かい日記
- 音声認識の誤変換は全て農業文脈で補正した上で日記を書くこと（「琵琶」→「枇杷」、「お昼をまいた」→「肥料を散布した」等）
- 事実復唱ではなく、感想・共感・明日への意欲を込める
- トーン: 「〜だったね」「〜お疲れさま！」のように寄り添う口調
- confidence "low"でも短くて良いので必ず生成する
- 例: 「暑い中の灌水、ほんとに大変だったね。でもハウスの枇杷たち、きっと喜んでるよ。明日も無理せずいこう！」

■ insight（気づきメモ、30文字以内）:
- 入力データから未来への教訓や観察ポイントを1行で抽出
- 音声認識の誤変換は農業文脈で補正すること
- 深い気づきが無い場合はnullを返す
- 例: 「灌水後の温度変化を要観察」「BMようりん効果検証は2週間後」`,
  ...
  OUTPUT_SCHEMA: `【出力JSON — status は必ず "complete"】
{
  ...
  "narrative": "str (相棒としての温かい振り返り日記、150文字以内。必ず生成)",
