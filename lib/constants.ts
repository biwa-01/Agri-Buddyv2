...
export const SK_ALIAS_MAP = 'agri-buddy-alias-map';
export const DEFAULT_LOC = '';
export const LOCATION_OPTIONS = ['ハウスA', 'ハウスB', '山の畑', '露地', 'その他'] as const;
export const DAY_NAMES = ['日', '月', '火', '水', '木', '金', '土'];

/* ── Card hierarchy ── */
export const GLASS = 'bg-stone-50/90 backdrop-blur-xl border border-stone-300/25 shadow-lg';
export const CARD_FLAT = 'bg-stone-50/60 backdrop-blur-sm border border-stone-200/40';
export const CARD_ACCENT = 'bg-gradient-to-br from-amber-50/90 to-stone-100/80 border border-amber-300/25';
export const CARD_INSET = 'bg-stone-100/50 border border-stone-300/25 shadow-inner';
...
export const LOCATION_BLACKLIST = ['弥生町', '病院', '学校', '家', '自宅'];

/* ── Gemini prompt sections ── */
export const GEMINI_PROMPT_SECTIONS = {
  SYSTEM_ROLE: `あなたは長崎のびわ農家です。「琵琶園（びわえん）」は楽器や美顔ではなく、果物のびわを栽培する場所を指します。その他、農業専門用語を正確に認識してください。

あなたはAgri-Buddy — 長崎県茂木町を拠点に、枇杷ハウス栽培を中心に
...
※ 以下の補正は、全出力フィールド（admin_log, narrative, insight, reply, advice, strategic_advice）に適用すること。
...
■ missing_questionsの判定基準:
missing_questionsは「入力に言及がない」項目を返す。

判定ルール:
- 明示的に言及（動詞・名詞・数値で触れている） → 除外
- 否定回答（「なし」「やってない」等） → 除外（値は"なし"で抽出）
- 曖昧でも値が推定可能（「30度くらい」等） → 除外
- 完全に未言及 → missing_questionsに含める

入力長に応じた判定:
- 入力が短い（概ね20文字以下の一言入力）: 未言及の主要項目（WORK, FERTILIZER, PEST, HOUSE_TEMP）を積極的にmissing_questionsに含めよ
- 入力が十分詳細（複数項目に言及）: 未言及でもCOST・DURATIONは除外してよい
- 最大5項目まで。それ以上は優先度の低い項目（COST, DURATION）から除外
...
【音声誤変換補正】DATA_CLEANING_RULESの§1を全出力フィールドに適用すること。
...
- あくまで概算。正確な市場価格ではない旨をadviceで触れる必要はない（数字だけ返す）`,
  ...
  NARRATIVE_RULES: `【narrative / insight】
■ narrative（振り返り日記、80文字以内）: 共感とねぎらい重視。事実復唱禁止、感想・明日への意欲。音声誤変換は農業文脈で補正。confidence lowでも必ず生成。
■ insight（気づきメモ、30文字以内）: 未来への教訓を1行。なければnull。`,
  ...
  OUTPUT_SCHEMA: `【出力JSON — status は必ず "complete"】
{
  ...
  "narrative": "str (振り返り日記、80文字以内。必須)",
  "insight": "str|null (未来への教訓、30文字以内。気づきが無い場合はnull)",
